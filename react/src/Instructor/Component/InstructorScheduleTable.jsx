...
export default function InstructorScheduleTable() {
    ...
    const columns = [
        {
            ...
            key: 'slot',
            width: 80,
            render: (text) => <div className="instructor-slot-cell">{text}</div>,
        },
        ...weekDates.map((date, index) => ({
            title: (
                <div className="instructor-day-header">
            ...
            render: (text) => {
                if (!text) return null
                return (
                    <div className="instructor-class-cell">
                        <div className="instructor-class-name">{text.name}</div>
                        <div className="instructor-class-info">at {text.location}</div>
                        <div className="instructor-class-info">{text.instructor}</div>
                        <div className="instructor-class-time">{text.time}</div>
                    </div>
                )
            },
        })),
    ...
    return <Table className="instructor-schedule-table" columns={columns} dataSource={data} pagination={false} bordered />
}